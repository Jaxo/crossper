define(["jQuery",
        ...
        "backbone",
        "models/offer",
		"text!templates/web/addOffer.html" ],
		function($, _, Backbone,offer,addOffer) {

	var crossper = crossper || {};

	crossper.AddOfferView = Backbone.View.extend({
		
		addOfferTemplate : Handlebars.compile(addOffer),
		...
		initialize : function() {
			...
			this.model.set({businessId:window.businessInfo.businessId});
		},

		render : function() {
			this.$el.html(this.addOfferTemplate());
...
//			$(ele).next("span").show();
		...
		createOffer:function(){
			...
			this.event.trigger("addNewOfferToCurrentOffers",this.model);
			/*var self=this;
			var options = {
